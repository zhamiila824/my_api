...
def show(id):
    resp = []
    for i in cursor.execute("""SELECT * FROM productsPositive WHERE id='{}'""".format(id)).fetchall():
        resp.append({
            "id": i[0],
            "product_name": i[1],
            "price": i[2],
...
def add():
    try:
        ...
        cursor.execute("""INSERT INTO productsPositive
              ...
              '{active}')""".format(id=req['id'], product_name=req['product_name'],
...
def search():
    req = json.loads(request.data)
    resp = []
    for i in cursor.execute("""SELECT * FROM productsPositive WHERE product_name LIKE '%{}%'""".format(req['product_name'])).fetchall():
        resp.append({
            "id": i[0],
            "product_name": i[1],
            "price": i[2],
